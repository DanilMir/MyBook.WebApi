...
type ProfileTests(factory: MyBookWebApplicationFactory) =
    class
        ...
        member this. ``GetCurrentUser`` () =
            ...
            let response = client.GetAsync($"/Profile/GetCurrentUser")
        ...
        member this. ``Change password with valid password`` () =
            ...
            let contestObject = new ChangePasswordViewModel()
            contestObject.NewPassword <- "qwe123QWE__";
            contestObject.OldPassword <- "qwe123QWE_"
            
            let contestJson = JsonConvert.SerializeObject(contestObject)
            let content = new StringContent(contestJson, Encoding.UTF8, "application/json");
            
            let response = client.PostAsync($"/Profile/ChangePassword", content)
            Assert.Equal(HttpStatusCode.OK, response.Result.StatusCode)
        ...
        member this. ``Remove From Favorites`` () =
            ...
            let id = "3cb92c37-ec67-4720-af23-d7f4d4096109"
            
            let response = client.GetAsync($"/Profile/RemoveFromFavorites?id={id}")
            Assert.Equal(HttpStatusCode.OK, response.Result.StatusCode)
        ...
        member this. ``Get Favorites`` () =
            ...
            let response = client.GetAsync($"/Profile/Favorites")
